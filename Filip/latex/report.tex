...
\]
\end{definition}
Hausdorff dimension is more rigorous and captures geometric scaling, but it is more difficult to estimate numerically. However, we can use the following inequality to bound it from above.
\[
...
The Hausdorff dimension of a measure \(\mu\) can be defined by considering the set with the smallest dimensions that contains almost all of the mass.
...
\]
\end{definition}
\noindent There are several other dimensions of measures. The following two are particularly important for numerical approximations.
...
\begin{theorem}[Young's dimension criterion]
...
for \(\mu\)-almost every \(x\).
\end{theorem}
Birkhoff's egodic theorem relates the average behaviour along a single trajectory to the space average of the whole system.
...
The numerical investigation of the H\'enon map
\[
...
Uneven mass concentration since for larger \(q\), \(D_q\) gives more weight to regions with larger mass.

\subsection{Local dimensions vs orbit length}
...
\label{tab:local-N}
\end{table}
The mean stays close to the global dimension estimates as $N$ increases, while the spread decreases, which shows exact dimensionality of the H\'enon map.

\subsection{Random petrubations}
...
\label{tab:sigma}
\end{table}
Clear increasing trend. Useless result? Always converges to 2 but with different speed.
